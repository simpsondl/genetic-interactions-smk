...
infile <- get_input()
outfile <- get_output()
message(sprintf("[%s] call_differential_hits.R starting; infile=%s, outfile=%s", Sys.time(), infile, outfile))

tab <- read_tsv(infile, show_col_types = FALSE)

# Determine discriminant column
...
if("Discriminant" %in% discr_cols) discr_col <- "Discriminant"
else if("Discriminant.score" %in% discr_cols) discr_col <- "Discriminant.score"
else discr_col <- discr_cols[1]
message(sprintf("[%s] Using discriminant column: %s", Sys.time(), discr_col))

# Read threshold param (quantile)
...
# Baseline categories: X+NT or NT+NT
