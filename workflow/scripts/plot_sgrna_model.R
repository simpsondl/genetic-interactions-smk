...
parp1_gamma_model <- read_tsv(snakemake@input[["input_gamma"]], 
...
ggsave(snakemake@output[["output_gamma_plot"]], gamma_plt, 
       device = "svg", height = 6, width = 7)


################# TAU PLOT #################

parp1_tau_model <- read_tsv(snakemake@input[["input_tau"]], 
                              show_col_types = FALSE)
...
ggsave(snakemake@output[["output_tau_plot"]], tau_plt, 
       device = "svg", height = 6, width = 7)