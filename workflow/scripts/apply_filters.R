...
# Support both a plain TSV or a ZIP containing the TSV (expected to contain *_raw_counts.tsv)
if (grepl("\\.zip$", input_counts_path, ignore.case = TRUE)) {
  zlist <- utils::unzip(input_counts_path, list = TRUE)
  # prefer a file that ends with _raw_counts.tsv
...
## Resolve counts columns by name-prefixes supplied in params.counts_prefixes
## Accept either a YAML/JSON list or a single comma-separated string.
prefixes_param <- snakemake@params[["counts_prefixes"]]
res <- resolve_count_prefixes(colnames(counts), prefixes_param)
...
sgrna_filt <- filt_low_representation(counts = counts,
                                      conds = cond_df,
                                      filtersamp = snakemake@params[["start_of_screen"]],
...
to_filt$Flag[to_filt$FirstPosition %in% sgrna_filt | to_filt$SecondPosition %in% sgrna_filt] <-
  "Low median representation of involved sgRNA at T0"
...
message(sprintf("[%s] Wrote filter flags to %s", 
                Sys.time(), snakemake@output[["output_filter_flags"]]))